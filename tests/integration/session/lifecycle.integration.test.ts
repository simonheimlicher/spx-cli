...
describe("Feature: Session Lifecycle", () => {
  ...
  describe("FI6: Handoff with frontmatter in content", () => {
    it("GIVEN content with frontmatter WHEN handoff THEN session preserves metadata", async () => {
      // Given: Content with frontmatter (simulating agent stdin)
...
Add JWT-based auth.`;
      ...
      // Write content to a temp file to simulate stdin
      const inputFile = join(sessionsDir.root, "input.md");
      await writeFile(inputFile, contentWithFrontmatter, "utf-8");

      // When: Create session file directly (simulating handoff)
      const sessionId = "2026-01-15_10-00-00";
      const sessionPath = join(sessionsDir.todo, `${sessionId}.md`);
      await writeFile(sessionPath, contentWithFrontmatter, "utf-8");

      // Then: Session file contains the frontmatter
      const savedContent = await readFile(sessionPath, "utf-8");
      expect(savedContent).toContain("priority: high");
      expect(savedContent).toContain("tags: [feature, api]");
      expect(savedContent).toContain("# Implement Authentication");
    });

    it("GIVEN content without frontmatter WHEN handoff adds defaults THEN session has default priority", async () => {
      // Given: Content without frontmatter
      const contentWithoutFrontmatter = `# Fix Login Bug

The login button doesn't work on mobile.`;

      // Simulate buildSessionContent adding defaults
...
${contentWithoutFrontmatter}`;
      ...
      // When: Create session file
      const sessionId = "2026-01-15_10-01-00";
      const sessionPath = join(sessionsDir.todo, `${sessionId}.md`);
      await writeFile(sessionPath, contentWithDefaults, "utf-8");

      // Then: Session file has default frontmatter
      const savedContent = await readFile(sessionPath, "utf-8");
      expect(savedContent).toContain("priority: medium");
      expect(savedContent).toContain("# Fix Login Bug");
    });
  });
});
